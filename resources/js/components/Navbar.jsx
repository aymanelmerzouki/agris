...
export default function Navbar() {
    ...
    return (
        <>
            {/* Desktop */}
            <header className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-100 dark:border-slate-800 shadow-sm hidden md:flex items-center justify-between px-6 py-3">
                        ...
                        <Link key={to} to={to}
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200
                                ${isActive(to)
                                    ...
                                    : 'text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-white hover:bg-green-50 dark:hover:bg-slate-800}'}`}>
                            <Icon size={16} />
                            <span>{label}</span>
                        </Link>
                    ))}
            ...
            {/* Mobile bottom nav */}
            <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-t border-gray-100 dark:border-slate-800 flex md:hidden">

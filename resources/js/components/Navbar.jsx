...
export default function Navbar() {
    ...
    return (
        <>
                        ...
                        <Link key={to} to={to}
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200
                                ${isActive(to)
                                    ...
                                    : 'text-gray-500 dark:text-green-300 hover:text-green-600 dark:hover:text-white hover:bg-green-50 dark:hover:bg-green-900'}`}>
                            <Icon size={16} />
                            <span>{label}</span>
                        </Link>
                    ))}
            ...
            {/* Mobile bottom nav */}
            <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 dark:bg-green-950/90 backdrop-blur-md border-t border-gray-100 dark:border-green-900 flex md:hidden">

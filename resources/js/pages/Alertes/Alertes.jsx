            ...
            <div className="max-w-5xl mx-auto px-4 md:px-6 mt-6">
                {alertes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
                        ...
                        <p className="text-lg font-bold text-gray-800 dark:text-zinc-100 dark:text-zinc-100">Aucune alerte pour le moment</p>
                        <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1">Vous êtes à jour. Les nouvelles alertes apparaîtront ici.</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                        {alertes.map((a) => (
                            <div key={a.id} className={`rounded-2xl p-4 shadow-sm border ${a.read_at ? 'bg-white dark:bg-zinc-900 border-gray-100 dark:border-zinc-800' : 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-700'}`}>
                                <p className="font-medium text-gray-800 dark:text-zinc-100">{a.data?.message}</p>
                                <p className="text-xs text-gray-400 dark:text-zinc-400 mt-1">{new Date(a.created_at).toLocaleDateString('fr-FR')}</p>
                            </div>
                        ))}
                    </div>
                )}

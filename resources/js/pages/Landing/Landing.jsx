...
export default function Landing() {
    ...
    return (
        <div className="min-h-screen bg-white pb-16 md:pb-0">
            ...
            <section className="py-16 max-w-6xl mx-auto px-6" id="actualites">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900">📰 Actualités agricoles</h2>
                        <p className="text-gray-500 text-sm mt-1">Les dernières nouvelles du secteur agricole marocain</p>
                    </div>
                    <span className="text-xs bg-green-100 text-green-700 px-3 py-1 rounded-full font-medium">En direct</span>
                </div>

                {/* Article principal */}
                {news[0] && (
                    <a href={news[0].url} target="_blank" rel="noreferrer"
                        className="block mb-6 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition group">
                        <div className="relative h-64 md:h-80">
                            <img src={news[0].image || 'https://images.unsplash.com/photo-1464226184884-fa280b87c399?w=800'}
                                alt={news[0].title}
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                                <span className="text-xs bg-green-500 px-2 py-1 rounded-full mb-2 inline-block">À la une</span>
                                <h3 className="text-xl md:text-2xl font-bold leading-tight mb-2">{news[0].title}</h3>
                                <p className="text-gray-300 text-sm line-clamp-2">{news[0].description}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {news.slice(1).map((article, i) => (
                        <a key={i} href={article.url} target="_blank" rel="noreferrer"
                            className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow hover:shadow-md transition group">

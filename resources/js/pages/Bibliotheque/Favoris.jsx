            ...
            <div className="max-w-5xl mx-auto px-4 md:px-6 mt-6">
                {plantes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
                        <Bookmark size={56} className="text-gray-300 dark:text-emerald-300 mb-4" />
                        <p className="font-semibold text-gray-600 dark:text-emerald-300">Votre sélection est vide.</p>
                        <p className="text-sm text-gray-400 dark:text-emerald-300 mt-1 mb-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {plantes.map((p) => (
                            <div key={p.id} className="bg-white dark:bg-emerald-900 rounded-2xl shadow-sm border border-gray-100 dark:border-emerald-800 overflow-hidden hover:shadow-md transition">
                                {p.imageUrl && (
                                ...
                                <div className="p-4">
                                    <Link to={`/plantes/${p.id}`}
                                        className="font-bold text-gray-800 dark:text-emerald-100 text-base hover:text-green-600 dark:hover:text-green-300 transition block">
                                        {p.nom}
                                    </Link>
                                    <p className="text-sm text-gray-500 dark:text-emerald-300/80 mt-0.5 italic">{p.espece}</p>
                                    {p.famille && <p className="text-xs text-gray-400 dark:text-emerald-300/70 mt-0.5">{p.famille}</p>}

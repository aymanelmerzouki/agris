            ...
            <div className="max-w-2xl mx-auto px-4 md:px-6 mt-6 space-y-4">
                ...
                {plante.description && (
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-zinc-800">
                        <h3 className="font-semibold text-gray-800 dark:text-zinc-100 mb-2">Description</h3>
                        <p className="text-sm text-gray-600 dark:text-zinc-400">{plante.description}</p>
                    </div>
                )}

                {plante.conditionsCulture && (
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-zinc-800">
                        <h3 className="font-semibold text-gray-800 dark:text-zinc-100 mb-2">Conditions de culture</h3>
                        <p className="text-sm text-gray-600 dark:text-zinc-400">{plante.conditionsCulture}</p>
                    </div>
                )}
            </div>
        </div>
...
function Info({ label, value }) {
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-3 shadow-sm border border-gray-100 dark:border-zinc-800">
            <span className="text-gray-400 dark:text-zinc-400 text-xs">{label}</span>
            <p className="font-semibold text-gray-800 dark:text-zinc-100 mt-0.5">{value ?? '—'}</p>
        </div>
    );
}
